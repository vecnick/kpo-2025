...
public class OutboxTaskService implements IOutboxTaskService {
    ...
    public Optional<OutboxTask> createOutboxTask(String requestPayload, TaskType taskType) {
    ...
    @Override
    public Optional<List<OutboxTask>> getOutboxTasksByStatus(OutboxStatus status) {
        try {
            return Optional.of(outboxTaskRepository.getOutboxTasksByStatus(status));
        } catch (Exception e) {
            System.out.println("OutboxTaskService: getOutboxTasksByStatus: не удалось получить запросы по статусу");
            return Optional.empty();
        }
    }

    @Override
    public Optional<OutboxTask> setStatusById(int id, OutboxStatus status) {
        try {
